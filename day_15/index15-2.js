...
// create empty array
for (let i = 0; i < rawMap.length * 5; i++) {
    map.push([]);
    for (let j = 0; j < rawMap[0].length * 5; j++) {
        map[i].push(undefined);
    }
}

// fill array according to the rules
for (let i = 0; i < map.length; i++) {
    for (let j = 0; j < map[0].length; j++) {

        const col = j % rawMap[0].length;
        const row = i % rawMap.length;
        
        const colTile = Math.floor(j / rawMap[0].length);
        const rowTile = Math.floor(i / rawMap.length);

        map[i][j] = parseInt(rawMap[row][col]) + colTile + rowTile;
...
while (priorityQueue.length > 0) {
    ...
    values.forEach(v => {
        ...
        // - if the new node is better, update the priorityQueue
        // - if it does not exist, insert at correct place
...
console.log(cost[`${map.length - 1},${map[0].length - 1}`]); // 2993